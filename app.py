...
def extract_features(img_gray):
    # Görüntüyü normalize etme ve GLCM hesaplama
...
if not assets:
    st.error("⚠️ Kritik Hata: Model dosyaları (.pkl) bulunamadı. Lütfen dosyaları uygulama dizinine yükleyin.")
else:
    model, scaler = assets
    ...
    if uploaded and user_name:
        ...
        with st.spinner("Yapay Zeka Mikroskobik Verileri İnceliyor..."):
            ...
            with col_res1: 
                st.image(uploaded, caption="Giriş Verisi", use_container_width=True)
            with col_res2: 
                st.image(res_img, caption="Analiz Sonucu", use_container_width=True)
